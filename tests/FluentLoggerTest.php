...
class FluentLoggerTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testPostWillReturnTrueInTheCaseOfPostingSuccessfully()
    {
        ...
        $logger     = Logger::open("localhost");
        $reflection = new \ReflectionProperty("DQNEO\Fluent\Logger", "socket");
...
//    }
    ...
    {
        return $this->getMock("DQNEO\Fluent\Logger", array("write"), array("localhost"));
    }
    ...
    {
        $actual_uri = Logger::getTransportUri($host, $port);
    ...
    {
        try {
            Logger::getTransportUri("udp://localhost", 1192);
    ...
    {
        $prop = new \ReflectionProperty("\DQNEO\Fluent\Logger", "instances");
    ...
    {
        $logger = new Logger("localhost", 119223);
    ...
    {
        $logger = new Logger("localhost", 119223);
        $this->assertEquals(Logger::CONNECTION_TIMEOUT, $logger->getOption("socket_timeout"),
